...
from nonebot.adapters import GroupMessageEvent, PrivateMessageEvent
from nonebot.adapters import GROUP_ADMIN, GROUP_OWNER
...
def get_path(*other):
    """获取数据文件绝对路径"""
    if config.arxiv_dir:
        dir_path = Path(config.arxiv_dir).resolve()
    else:
...
def on_startup():
    ...
    if config.fastapi_reload and sys.platform == "win32":
        raise ImportError("加载失败,Windows 必须设置 FASTAPI_RELOAD=false 才能正常运行 ArXiv-Bot")
